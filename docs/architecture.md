...
MVP では Relay と coturn を 自前サーバー (`your-host.example.com`) に Docker Compose でデプロイする。

- **DNS**: 動的 DNS で `codex-link-p2p.your-host.example.com` を自宅 router の WAN IP に向ける (HTTPS + TURN を 1 ホスト名に同居)
...
実機 iPhone (iOS 26.5、Wi-Fi/5G) と本番 Relay (`codex-link-p2p.your-host.example.com`) で
